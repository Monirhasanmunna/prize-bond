...
export default function Page({data: drawListData}){
    const [pagination, setPagination] = useState({page: drawListData.page, length: drawListData.length})
    const [searchText, setSearchText] = useState('')
    const [go, setGo] = useState(false)
    const [alertData, setAlertData] = useState({route : '', color : '', method : '', text : ''});

    useEffect(() => {
        setPaginationFromSessionStorage(setPagination, 'draw_pagination', {
            page: drawListData?.page, length: drawListData?.length, status: drawListData.status, searchText: ""
        }, )
    }, []);
    ...
    useEffect(() => {
        if (pagination?.page && pagination?.length && go) {
            ...
            router.get(route('admin.draw.list', queryParams))

            sessionStorage.setItem('draw_pagination', JSON.stringify(pagination))
        }
    }, [go]);
    ...
    const alert = (id) => {
        setAlertData({
            ...alertData,
            route : route('admin.draw.delete', id),
            color : 'red',
            method : 'delete',
            ...
            <Main>
                ...
                <div className="w-full border border-gray-300 rounded mt-5">
                    <div className="flex items-center justify-between border-b border-gray-300 px-5 py-3">
                        ...
                        <Link href={route('admin.draw.create')} className="py-1.5 px-5 inline-flex items-center gap-x-2 text-sm font-medium rounded bg-blue-600 text-white hover:bg-blue-700">
                            Add New
                        </Link>
                    </div>
                    ...
                    <div className="w-full p-5">
                        <div className="flex flex-col md:flex-row md:justify-between  items-center gap-3 my-4 text-[.75rem]">
                            ...
                            <div className="flex flex-col md:flex-row items-center md:justify-end gap-3 flex-wrap">
                                {/*<StatusFilter status={drawListData.drawStatus} pagination={pagination} setPagination={setPagination} setGo={setGo} segment="Series" />*/}
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full border border-neutral-300 dark:border-neutral-400 border-collapse text-sm">
                                ...
                                <tbody>
                                {
                                    drawListData.draws.map((draw, i) => (
                                        <tr key={draw.id} className={`hover:bg-gray-100 dark:hover:bg-neutral-600`}>
                                            ...
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                <div className="w-full flex gap-x-3">
                                                    ...
                                                    {/*        onClick={() => editSeries(draw)}>*/}
                                                    {/*    <MdEditSquare className={`text-blue-400 text-lg`}/>*/}
                                                    {/*</button>*/}
                                                    <button className=" cursor-pointer text-white border-green-600"
                                                            onClick={() => alert(draw.id)}>
                            ...
                            <div className="py-3 sm:px-6">
                                <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
                                    <Summary
                                        page={drawListData.page}
                                        length={drawListData.length}
                                        count={drawListData.count}
                                    />
                                    <Navigation
                                        page={pagination.page}
                                        length={drawListData.length}
